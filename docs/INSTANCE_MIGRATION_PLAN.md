...
Replacing an existing D1 ID is treated as a data migration:
...
Assigning the first D1 ID to an unprovisioned manifest is treated as initial provisioning: create or verify the database, apply its schema and confirm the binding without inventing a previous-data copy.
