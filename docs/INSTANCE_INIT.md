...
The two rate-limit IDs must be distinct, non-zero positive integer strings.
...
All instance IDs and Cloudflare resource names are validated before any file is written. Names must start and end with a lowercase letter or number; R2 bucket names must be 3–63 characters.
...
- not contain a `.git`, `.generated` or `node_modules` path segment, regardless of case
- not traverse an existing symbolic link or Windows junction
