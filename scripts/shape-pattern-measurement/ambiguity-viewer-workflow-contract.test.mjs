...
describe('credentialed Shape ambiguity viewer workflow', () => {
  it('is manual, main-only, bounded, and uploads no raw TDX cache', async () => {
    ...
    expect(workflow).toContain('scripts/shape-pattern-measurement/ambiguity-viewer.mjs')
    expect(workflow.indexOf('unset TDX_CLIENT_ID TDX_CLIENT_SECRET'))
      .toBeLessThan(workflow.indexOf('scripts/shape-pattern-measurement/ambiguity-viewer.mjs'))
    ...
    expect(workflow).not.toContain('run.mjs')
  })
})
