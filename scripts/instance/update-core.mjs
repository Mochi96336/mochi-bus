...
export async function writeInstanceUpdate(result) {
  ...
  if (
    ...
    || (currentMetadata.mode & 0o7777) !== result.sourceIdentity.mode
  ) {
  ...
  try {
    ...
    await handle.writeFile(serializeManifest(result.manifest, result.format), 'utf8')
...
export function renderInstanceUpdateText(result, { written = false } = {}) {
  ...
  if (!written) {
    ...
    if (result.changed) lines.push('Re-run the command with --write to rebuild and apply the update against the current file.')
...
async function loadEditableManifest(configPath, { cwd }) {
  ...
  assertNoReservedDirectory(logicalRelative)
  ...
  assertNoReservedDirectory(physicalRelative)
...
function assertNoReservedDirectory(pathFromRoot) {
  const reserved = pathFromRoot
    .split(sep)
    .find((segment) => RESERVED_DIRECTORIES.has(segment.toLowerCase()))
  if (reserved) throw new Error(`Instance manifest cannot be updated inside ${reserved}`)
}
