...
describe('instance initializer repository contract', () => {
  ...
  test('preserves the public route-name boundary in schema and validation', async () => {
    const schema = JSON.parse(await read('config/instance.schema.json'))
    const routeName = schema.properties.transit.properties.demoQuery.oneOf[1].properties.routeName
    const validator = await read('scripts/instance/config.mjs')

    expect(routeName.maxLength).toBe(40)
    expect(validator).toContain('nonEmptyString(query.routeName, `${path}.routeName`, errors, 40)')
  })
