...
function fakeFileStat({ size, mtimeNs = 1n, ctimeNs = 1n }) {
  return {
    dev: 1n,
    ino: 2n,
    size,
...
describe('instance bundle artifacts', () => {
  ...
  test('rejects traversal, manifest replacement, reserved generated names and non-JSON output', () => {
    ...
    expect(() => resolveBundleArtifactOutputPath('/repo', '.Git/bundle.json', 'instance.json')).toThrow('.git')
    expect(() => resolveBundleArtifactOutputPath('/repo', 'NODE_MODULES/bundle.json', 'instance.json')).toThrow('node_modules')
  ...
  test('applies pre-read limits and rejects symlink inputs', async () => {
    await withWorkspace(async (cwd) => {
  ...
  test('caps growing reads and rejects same-size mutation', async () => {
    let requestedLength = null
    const growingHandle = {
      async stat() { return fakeFileStat({ size: 4n }) },
      async read(buffer, offset, length) {
        requestedLength = length
        buffer.fill(0x7b, offset, offset + length)
        return { bytesRead: length, buffer }
      },
      async close() {},
    }
    await expect(readInstanceBundleArtifact('growing.json', {
      cwd: '/repo',
      maxBytes: 8,
      openFile: async () => growingHandle,
    })).rejects.toThrow('8-byte read limit')
    expect(requestedLength).toBe(9)
    ...
    await expect(readInstanceBundleArtifact('changing.json', {
      cwd: '/repo',
      maxBytes: 8,
      openFile: async () => changingHandle,
