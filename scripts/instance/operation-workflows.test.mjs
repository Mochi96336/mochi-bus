...
describe('instance operational workflow gates', () => {
  it('resolves scheduled snapshot scope before migrations and publication', () => {
    ...
    expect(syncWorkflow.indexOf(scope)).toBeLessThan(syncWorkflow.indexOf('wrangler d1 migrations apply'))
    expect(syncWorkflow.match(new RegExp(condition.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'))).toHaveLength(2)
  })

  it('skips disabled public probe and watchdog jobs before D1 access', () => {
    for (const [source, operation] of [
      ...
      expect(source.indexOf(scope)).toBeLessThan(source.indexOf('wrangler d1 migrations apply'))
      expect(source.match(/if: steps\.operation\.outputs\.enabled == 'true'/g)).toHaveLength(2)
    }
  })
