...
describe('instance operational workflow gates', () => {
  it('resolves and preflights snapshot scope before migrations while deferring TDX auth to publication', () => {
    const scope = 'node scripts/instance/operation-scope.mjs snapshot'
    const preflight = 'npm run instance:preflight -- snapshot'
    const eagerTokenPreflight = 'node scripts/transit-snapshot/prepare-snapshot-tdx-token.mjs'
    ...
    expect(syncWorkflow).not.toContain(eagerTokenPreflight)
