...
const INSTANCE_ID_PATTERN = /^[a-z](?:[a-z0-9-]{1,61}[a-z0-9])$/
const CLOUDFLARE_NAME_PATTERN = /^[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?$/
...
const RATE_LIMIT_NAMESPACE_ID_PATTERN = /^[1-9][0-9]{0,19}$/
...
export function buildInstanceManifest(options, { cwd = process.cwd() } = {}) {
  ...
  if (!INSTANCE_ID_PATTERN.test(instanceId)) {
    throw new Error('--instance-id must start with a lowercase letter, end with a lowercase letter or number, and contain only lowercase letters, numbers or hyphens (3-63 characters)')
  }
  ...
  const outputPath = resolveInitOutputPath(repositoryRoot, options.outputPath || 'instance.json')
  const manifest = {
    $schema: schemaReference(repositoryRoot, outputPath),
  ...
  return Object.freeze({
    ...
    repositoryRoot,
    outputPath,
    displayPath: displayPath(repositoryRoot, outputPath),
    profile,
    strictValidation,
...
export async function writeInstanceManifest(result, { force = false } = {}) {
  ...
  await assertSafeOutputAncestors(result.repositoryRoot, result.outputPath)
...
function resolveInitOutputPath(cwd, value) {
  ...
  if (!isPathInside(root, target) || !pathFromRoot) {
  ...
  const reservedSegment = pathFromRoot
    .split(sep)
    .find((segment) => RESERVED_OUTPUT_DIRECTORIES.has(segment.toLowerCase()))
  if (reservedSegment) throw new Error(`--output cannot write inside ${reservedSegment}`)
  return target
}

async function assertSafeOutputAncestors(repositoryRoot, outputPath) {
  const lexicalRoot = resolve(repositoryRoot)
  const resolvedRoot = await realpath(lexicalRoot)
  const pathFromRoot = relative(lexicalRoot, outputPath)
  ...
  for (const segment of pathFromRoot.split(sep).slice(0, -1)) {
    ...
    if (stats.isSymbolicLink()) {
      throw new Error('--output cannot traverse symbolic links or junctions')
    }
    if (!stats.isDirectory()) {
      throw new Error('--output parent path must contain only directories')
    }
    current = await realpath(current)
    if (!isPathInside(resolvedRoot, current) && current !== resolvedRoot) {
...
function validateCloudflareName(value, optionName, pattern, lengthDescription) {
  if (!pattern.test(value)) {
    throw new Error(`${optionName} must start and end with a lowercase letter or number and contain only lowercase letters, numbers or hyphens (${lengthDescription})`)
  }
  return value
}
