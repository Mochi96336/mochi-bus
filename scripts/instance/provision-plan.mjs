...
function d1Step({ doctor, remote, configDisplayPath, resourceIdentity }) {
  ...
  if (!name) {
    return {
      ...
      detail: 'cloudflare.d1.databaseName is missing or invalid.',
    }
  }
  if (!id) {
    return {
...
function r2Step({ doctor, remote, resourceIdentity }) {
  ...
  if (!name) {
    return {
      ...
      detail: 'cloudflare.r2.bucketName is missing or invalid.',
    }
  }
...
function repositorySecretSteps({ env, profile, operations }) {
  ...
  for (const spec of SECRET_SPECS) {
    ...
    steps.push({
      ...
      status: configured ? 'complete' : optional ? 'optional' : 'action_required',
      title: `Set GitHub secret ${spec.name}`,
      detail: configured
        ? `${spec.name} is present in the current execution context.`
        : optional
...
function repositoryVariableSteps({ env, config, configDisplayPath, operations }) {
  ...
  if (normalizePath(configDisplayPath) !== defaultProduction) {
    ...
    steps.push({
      ...
      status: configured ? 'complete' : 'action_required',
      title: 'Select the fork instance manifest in GitHub Actions',
      detail: configured
        ? 'MOCHI_BUS_INSTANCE_CONFIG is present in the current execution context.'
...
function originVariableStep({ env, name, consumer }) {
  ...
  return {
    ...
    detail: configured
      ? `${name} is present in the current execution context.`
    ...
    manualActions: configured ? [] : ['Replace the placeholder with the exact public HTTPS origin.'],
...
function resolveResourceIdentity(config) {
  return Object.freeze({
    d1Name: stringValue(config.cloudflare?.d1?.databaseName),
    d1Id: stringValue(config.cloudflare?.d1?.databaseId),
    r2Name: stringValue(config.cloudflare?.r2?.bucketName),
