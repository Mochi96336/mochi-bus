...
export async function readInstanceBundleArtifact(inputPath, {
  ...
  if (!Number.isSafeInteger(maxBytes) || maxBytes < 1) {
    throw new Error('Bundle artifact read limit must be a positive safe integer')
  }
  const resolvedPath = resolve(cwd, inputPath)
  let handle
  try {
    const noFollow = typeof constants.O_NOFOLLOW === 'number' ? constants.O_NOFOLLOW : 0
    handle = await openFile(resolvedPath, constants.O_RDONLY | noFollow)
    const before = await handle.stat({ bigint: true })
    if (!before.isFile()) throw new Error('Bundle artifact input must be a regular file')
    if (before.size === 0n) throw new Error('Bundle artifact input is empty')
    if (before.size > BigInt(maxBytes)) {
      throw new Error(`Bundle artifact exceeds the ${maxBytes}-byte read limit`)
    }
    const { source, bytesRead } = await readBoundedArtifactSource(handle, maxBytes)
...
async function readBoundedArtifactSource(handle, maxBytes) {
