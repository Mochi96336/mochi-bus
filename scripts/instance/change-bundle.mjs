...
import { buildInstanceMigrationPlan } from './migration-plan.mjs'
...
export async function buildInstanceChangeBundle(options, {
  ...
  const migrationPlan = await buildInstanceMigrationPlan({ updateOptions: { ...updateOptions, write: false } }, { cwd, env })
  ...
  return deepFreeze({
    ...
    consistency: {
      sameProposal: true,
      proposalFingerprint,
      verification: 'canonical updater and migration-plan change fingerprints match',
    },
    proposal: {
      changed: proposal.changed,
...
function renderVerificationArguments(bundle) {
  const parts = ['--config', shellQuote(bundle.instance.configPath)]
  for (const change of bundle.proposal.changes) {
    const option = optionForPath(change.path)
    if (!option) continue
    parts.push(option, shellQuote(optionValue(change.after)))
  }
  return parts.join(' ')
}
...
function optionForPath(path) {
  const options = {
    'site.name': '--site-name',
    'site.canonicalOrigin': '--origin',
    'transit.enabledCities': '--cities',
    'transit.defaultCity': '--default-city',
    'cloudflare.workerName': '--worker-name',
    'cloudflare.d1.databaseName': '--d1-name',
    'cloudflare.d1.databaseId': '--database-id',
    'cloudflare.r2.bucketName': '--r2-name',
    'cloudflare.rateLimits.standardNamespaceId': '--standard-rate-limit-id',
    'cloudflare.rateLimits.expensiveNamespaceId': '--expensive-rate-limit-id',
    'cloudflare.workersDev': '--workers-dev',
    'operations.profile': '--profile',
    'operations.snapshotSchedule': '--snapshot-schedule',
    'operations.releaseSmoke': '--release-smoke',
    'operations.publicProbe': '--public-probe',
    'operations.windowWatchdog': '--window-watchdog',
  }
