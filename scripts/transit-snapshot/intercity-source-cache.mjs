  ...
  return Object.freeze({
    async resolve({ resource, input, init }) {
      try {
        ...
        const state = await storage.getJson(stateKey(resource), STATE_MAX_BYTES)
        if (!validState(state, resource) || state.sourceVersion !== sourceVersion) {
        ...
        if (digest !== state.sha256 || body.byteLength !== state.bytes) {
          logger?.warn?.(`TDX InterCity persistent cache integrity mismatch for ${resource}`)
          return { body: null, sourceVersion }
        }
        ...
        logger?.warn?.(`TDX InterCity persistent cache read failed for ${resource}: ${errorMessage(error)}`)
    ...
    async store({ resource, body, sourceVersion }) {
      ...
      try {
        ...
        const payloadKey = `${CACHE_PREFIX}/${resource}/payload-${digest}.json`
        await storage.putBuffer(payloadKey, bytes, 'application/json')
        await storage.putJson(stateKey(resource), {
          schemaVersion: 1,
          resource,
          sourceVersion,
          payloadKey,
        ...
        logger?.warn?.(`TDX InterCity persistent cache write failed for ${resource}: ${errorMessage(error)}`)
        return false
      }
    },
  })
...
export function createR2IntercitySourceCache({ env = process.env, fetchImpl = globalThis.fetch, logger = console } = {}) {
  ...
  if (!accessKeyId || !secretAccessKey || !accountId || !bucket || typeof fetchImpl !== 'function') return null
...
export function intercityProbeUrl(input) {
...
async function probeSourceVersion(fetchImpl, input, init) {
  const url = intercityProbeUrl(input)
  ...
  const updateTime = Array.isArray(payload) ? nonEmpty(payload[0]?.UpdateTime) : null
  return updateTime
}

function stateKey(resource) {
  return `${CACHE_PREFIX}/${resource}/state.json`
}

function validState(value, resource) {
  return value && value.schemaVersion === 1
    && value.resource === resource
    && nonEmpty(value.sourceVersion)
    && typeof value.payloadKey === 'string'
    && value.payloadKey.startsWith(`${CACHE_PREFIX}/${resource}/payload-`)
